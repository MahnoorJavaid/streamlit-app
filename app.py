...
def analyze_test_images(images_data):
    """
    Analyze all test images through AI pipeline
    images_data: list of dicts with {topic, question_num, image_bytes}
    """
    from questions import MATH_QUESTIONS  # Import questions

    # Prompts for AI
   # Import prompts from prompts.py (already imported at top)
    # Use directly: QWEN_PROMPT, DEEPSEEK_PROMPT_1, DEEPSEEK_PROMPT_2
    ...
    for idx, img_data in enumerate(images_data):
        ...
        question_num = img_data['question_num']
        image_bytes = img_data['image_bytes']

        status_text.text(
            f"Processing {topic} - Question {question_num}... (Step 1/3)")

        # Convert to base64
        image_base64 = base64.b64encode(image_bytes).decode('utf-8')

       # Step 1: Qwen-VL extraction (text only, no question needed)
        status_text.text(
            f"Processing {topic} - Question {question_num}... (Extracting content)")
        qwen_output = process_image_with_qwen(image_base64, QWEN_PROMPT)

        # Step 2: DeepSeek analysis (with question + extracted content)
        status_text.text(
        ...
        # Combine question + extracted solution
        deepseek_input = f"""ORIGINAL QUESTION:
{actual_question}

STUDENT'S EXTRACTED SOLUTION:
{qwen_output}"""
...
def dashboard_page():
    ...
    if not tests:
        ...
        if latest_test.get('final_feedback'):
            ...
            st.warning(
                " AI analysis not available for latest test. Take a new test to see AI feedback!")
...
def test_page():
    """Test taking page with image upload for 12 questions"""
    user = st.session_state.user
    ...
    **Test Instructions:**
    ...
    - Upload a **clear image** of your handwritten solution for each question
    ...
    for topic in MATH_TOPICS:
        ...
        with col1:
            ...
            if uploaded_file1:
                ...
                image = Image.open(io.BytesIO(
                    st.session_state.test_images[key1]['image_bytes']))
        ...
        with col2:
            ...
            if uploaded_file2:
                ...
                # Show preview
                image = Image.open(io.BytesIO(
                    st.session_state.test_images[key2]['image_bytes']))
                st.image(image, use_column_width=True)
    ...
    st.write(f"**Uploaded: {uploaded_count}/12 questions**")

    if uploaded_count < 12:
        st.warning(
            f"⚠️ Please upload all 12 images. {12 - uploaded_count} remaining.")
        ...
        if st.button(" Submit Test for AI Analysis", use_container_width=True, type="primary", disabled=(uploaded_count < 12)):
            if uploaded_count == 12:
                ...
                with st.spinner("🤖 AI is analyzing your test... This may take a few minutes."):
                    ...
                    images_for_analysis = list(
                        st.session_state.test_images.values())

                    # Analyze with AI
                    result = analyze_test_images(images_for_analysis)
